...
const MOCK_CHATS = [
    {
        ...
        education: "MBBS, MD",
        compatibility: "92%",
        status: "accepted", // 'pending', 'accepted', 'sent'
        pinned: true,
...
const MessagingSystem = () => {
    ...
    const [chatStatus, setChatStatus] = useState("accepted"); // For demoing Interest Request
    ...
    useEffect(() => {
        // Update local chat state when selection changes
        if (activeChat) {
            setChatStatus(activeChat.status);
            if (activeChat.id === 1) setMessages(MOCK_MESSAGES);
            else setMessages([]); // Empty for others in this mock
        }
    ...
    const resize = (e) => {
        if (isResizing) {
            // Apply min and max width constraints to keep layout solid
